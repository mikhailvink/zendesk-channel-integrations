...
function write_log($message)
{
    ...
    if ($globalConfig['LogsDBType'] == '1' or $globalConfig['LogsDBType'] == '3') {
        if (!is_file("../logs/dockerhub_" . date("Y_m_d") . ".log")) {
            $fh = fopen("../logs/dockerhub_" . date("Y_m_d") . ".log", 'w');
            fclose($fh);
        }
        $logfile = "../logs/dockerhub_" . date("Y_m_d") . ".log";
...
VALUES (
'" . $date . "',  '" . $remote_addr . "',  '" . $request_uri . "', '" . $message . "',  'DockerHub',  '');");
...
function write_log_callback($message)
{
    ...
    if ($globalConfig['LogsDBType'] == '1' or $globalConfig['LogsDBType'] == '3') {
        if (!is_file("../logs_callback/dockerhub_" . date("Y_m_d") . ".log")) {
            $fh = fopen("../logs_callback/dockerhub_" . date("Y_m_d") . ".log", 'w');
            fclose($fh);
        }
        $logfile = "../logs_callback/dockerhub_" . date("Y_m_d") . ".log";
...
VALUES (
'" . $date . "',  '" . $remote_addr . "',  '" . $request_uri . "', '" . addslashes($message) . "',  'DockerHub',  '" . json_decode($message, true)['events'][0]['subdomain'] . "', '" . json_decode($message, true)['events'][0]['type_id'] . "', '".$error."');");
